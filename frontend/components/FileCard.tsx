...
export default function FileCard({ item, onOpen, onDelete, onOCR }: { item: { fileId: string; name: string; mime?: string }; onOpen: (id: string) => void; onDelete: (id: string) => void; onOCR: (id: string) => void }) {
  ...
  return (
    <div className="group rounded-lg border p-3 bg-white hover:shadow cursor-pointer relative">
      ...
      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition space-x-2">
        <button onClick={(e)=>{e.stopPropagation(); onOpen(item.fileId)}} className="px-2 py-1 text-xs bg-blue-600 text-white rounded">预览</button>
        <button onClick={(e)=>{e.stopPropagation(); onOCR(item.fileId)}} className="px-2 py-1 text-xs bg-amber-600 text-white rounded">OCR</button>
