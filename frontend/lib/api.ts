...
// OCR tasks
export async function createOCR(fileId: string): Promise<{ jobId: string }> {
  const url = `${API_BASE}/tasks/ocr`
  return to(await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders() }, body: JSON.stringify({ fileId }) }))
}

export async function getJob(jobId: string): Promise<{ jobId: string; status: string; resultURI?: string }> {
  const url = `${API_BASE}/tasks/${encodeURIComponent(jobId)}`
  return to(await fetch(url, { headers: { ...authHeaders() }, cache: 'no-store' }))
}
